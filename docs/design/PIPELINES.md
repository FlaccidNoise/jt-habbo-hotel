...
4. **Validation gates (automated).** Palette compliance, grid alignment, silhouette contrast
   against the darkest floor a room can lay, footprint sanity, animation bounds, **recipe-uniqueness**
   ...
   **similarity to previously rejected designs**, the reference-scene render gates (draw order,
   ...
   Gates run by surface. Palette and uniqueness are surface-blind. Footprint, seat and
   ...
   Flat decor is the third surface and replaces the geometry gates outright — it has no box, no
   footprint and no seat (§2a). `gateDecorLattice`, `gateDecorTiles` and `gateDecorContrast` are
   what it runs instead.

   **Contrast is now one comparison, not two (#260).** It used to name the two floor greens, which
   let a silhouette pass by being *brighter* than both. A decor floor may be any palette colour at
   or above `BACKDROP_LUMA_MIN` — the lightest outline any ramp can paint, plus `MIN_CONTRAST` —
   so "brighter than the floor" is a guarantee nothing can give any more. The surviving rule is
   that a silhouette is darker than the darkest floor. Every shipped bundle passes at luma 35.1,
   because `outlineSilhouette` paints every edge the global `OUTLINE`.
...
## 2a. Flat decor pipeline

Built 2026-08-05 (#260). Wallpapers and floor patterns — the third asset class, and the only one
that may be **raster**.

Why raster is safe here and nowhere else: a raster asset carries no `DepthBox`, so `gateDrawOrder`
and `gateSeatOcclusion` cannot cover it (#233, #235). That only matters for things standing on the
floor that avatars route around and sit on. A floor pattern *is* the floor and a wallpaper *is* the
wall — both are already the flat backdrop everything else sorts against, so there is no ordering
left to get wrong. The class therefore runs raster at full fidelity without weakening one
floor-furni invariant. It is also where an image model genuinely fits: the artgen quantizer works
only because its input is white geometry under one known sun, and a flat texture needs no such
structure — nearest-in-RGB is the whole job (`packages/generator/src/decor.ts`).

**The asset is one tile, and the client repeats it.** Nothing is pre-projected. The tile is
authored in the surface's own two axes and a texture matrix carries it into screen space, so the
same bytes serve the left wall, the right wall, and every raised floor height.

- **Floor** — the tile is screen-space and axis-aligned, filled with `textureSpace: "global"` so it
  is laid down once across the whole floor plane and each diamond cuts its own window. Diamonds
  tessellate on the lattice generated by (+32, +16) and (−32, +16), and a w × h rectangle repeats
  compatibly with it exactly when w is a multiple of 64 and h a multiple of 32. That is why the
  cell is two diamonds wide — the old two-tone checker is the smallest legal pattern. A raised tile
  keeps its phase for free: its lift is a whole number of 32 px height units.
- **Wall** — the tile is wall-space (u along the wall, v straight down), the axes walls.ts already
  measures hanging items in. The fill matrix `(sign, 0.5, 0, 1, o.sx, o.sy)` *is* `wallOffset`, and
  the GPU's nearest sampling produces the 2:1 stair-step. Anchored per segment, so the tile width
  must divide `WALL_SEG_PX` and be even — an odd repeat lands the pattern half a pixel off itself,
  the same reason a hung item's `u` is even.

Authoring is `tools/decor/source/<id>.png` — a plain raster, so any image editor is the tool and
nothing in the pipeline cares how it was made. `make decor` quantizes, gates and freezes one tile
to `tools/decor/frozen/`; `make gen` reads the frozen bytes, verifies the hash, re-runs the gates
and publishes. Pixels are the pattern's identity, exactly as for furni.

Three gates replace the geometry ones:

- `gateDecorLattice` — the declared tile lands on its surface's lattice. Pure def check; a wrong
  size shifts the pattern against itself forever and no raster can fix it.
- `gateDecorTiles` — the authored source **is its own tile, repeated, exactly**. This is the seam
  check, and it is exact rather than a threshold. Measuring a discontinuity at the wrap edge cannot
  work — a stripe pattern is a discontinuity by design, which is how the dead-band check died in
  #258 (see the note in `review.ts`). So the author supplies more than one period and the build
  proves the periods agree. Runs at authoring time only: the frozen artifact is one tile, which
  repeats by construction.
- `gateDecorContrast` — every tile pixel and every declared flat colour is opaque, on-palette, and
  at or above `BACKDROP_LUMA_MIN`. It covers avatars as well as furni: a figure layer's outline is
  its *worn* ramp's darkest shade, chosen by the player, so the check cannot know which and holds
  against all of them.

Both wall gates earned their keep on first run, unstaged: `wall_wainscot`'s panel groove quantized
to walnut left (luma 76.2, under the bound) and `wall_pinstripe`'s art repeated on 16 px while its
def still said 8. Neither is a defect a human would have caught by eye at 64.

Two flat colours ride on the def rather than the raster, because shearing a floor texture up a
riser or across a wall cap reads as a smear rather than as the same material turned on edge: a
floor decor declares its raised-tile `sides`, a wall decor its `cap`.
