...
describe("db", () => {
  test("seeds exactly the four public rooms, and reopening is idempotent", () => {
    ...
    expect(rooms[3]?.name).toBe("Resort Grounds");   // #406
    closeDb(db);

    db = openDb(dbPath);
    expect(db.prepare("SELECT id FROM rooms").all()).toHaveLength(4);
    closeDb(db);
  });
